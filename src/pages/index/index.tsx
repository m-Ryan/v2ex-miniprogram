...
class Index extends Component<IProps, IState> {
    ...
    render() {
        ...
        const renderHotNodes = data.hot_nodes.map((item, index) => {
            return (
                <Navigator
                    url={formatPath(Pages.NodeListIndex, {
        ...
        const renderTabBarList = data.tabs.map((item, index) => {
            return (
                <View
                    onClick={() => this.onChangeTabBar(item, index)}
                    className={
                        currentTabBar === index
                            ? formatClassName(
        ...
        const renderSecondTabBarList =
            data.secondary_tabs.length > 0 ? (
                data.secondary_tabs.map(item => {
                    return (
                        <Navigator
                            url={formatPath(Pages.NodeListIndex, {
                                path: getNodeName(item.url),
